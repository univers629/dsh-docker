...
$script:ModelApi = @('relay=responses')
$script:ModelHeader = @(
    'relay=user-agent=codex_cli_rs/0.101.0',
    'relay=originator=codex_cli_rs',
    'relay=version=0.101.0'
)
$script:ModelId = @('relay=gpt-fake-1,gpt-fake-2')
@(
    'relay',                          # 上游名字
    'https://relay.example.com',          # base_url（不在内置默认表里才会问）
    $codexKey,                            # 密钥
    $codexKey,                            # 密钥确认
    'n'                                   # 不再添加上游
...
Assert-Equal 'relay' $entry.name '上游名字'
Assert-Equal 'https://relay.example.com' $entry.baseUrl '上游 base_url'
...
Assert-Equal 'responses' (Get-BrokerUpstreamProfile 'relay') '向导记下的 API 形态'
...
Assert-Equal 'gpt-fake-1,gpt-fake-2' $script:BrokerModels['relay'] '向导沿用 -ModelId 的模型清单'
...
const upstream = parsed.upstreams.get('relay')
assert.ok(upstream, 'relay upstream missing')
assert.equal(upstream.baseUrl, 'https://relay.example.com')
...
$script:BrokerProfiles = @{}
$script:ModelApi = @('Relay=Responses')
$script:ModelHeader = @('relay=user-agent=codex_cli_rs/0.101.0', 'relay=originator=codex_cli_rs')
Test-ModelSpecFormat
Assert-Equal 'responses' (Get-ModelApiOverride 'relay') '-ModelApi 大小写归一'
# -ModelId 同一个上游可以给多条，也可以在一条里用逗号分隔，最后合成一条。
$script:ModelId = @('Relay=claude-opus-5-thinking', 'relay=gpt-5.2,gpt-5.2-codex')
Test-ModelSpecFormat
Assert-Equal 'claude-opus-5-thinking,gpt-5.2,gpt-5.2-codex' (Get-ModelIdOverride 'relay') '-ModelId 合并'
...
$headerOverrides = Get-ModelHeaderOverrides 'relay'
...
Add-BrokerUpstream -Name 'relay' -BaseUrl 'https://relay.example.com' -Key $codexKey `
    -ApiProfile (Get-ModelApiOverride 'relay') -ExtraHeader (Get-ModelHeaderOverrides 'relay')
...
# ---------------------------------------------------------------------------
foreach ($case in @(
    @{ Api = @('relay=grpc'); Header = @(); Match = '未知的 API 形态' },
    @{ Api = @('relay'); Header = @(); Match = 'NAME=PROFILE' },
    @{ Api = @(); Header = @('relay=user-agent'); Match = 'NAME=HEADER=VALUE' },
    @{ Api = @(); Header = @('relay=authorization=Bearer leaked'); Match = '由密钥代理自己管理' },
    @{ Api = @(); Header = @('relay=x-api-key=leaked'); Match = '由密钥代理自己管理' },
    @{ Api = @(); Header = @('relay=bad header=1'); Match = '不是合法的 HTTP 头名' }
)) {
