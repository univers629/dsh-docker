...
try {
  ...
  const relay = profiledParsed.upstreams.get('relay')
  assert.ok(relay, 'the relay upstream must exist')
  assert.equal(relay.headerValue, `Bearer ${codexKey}`)
  assert.ok(relay.allowedPathPrefixes.includes('/v1/responses'), 'responses profile must allow /v1/responses')
  assert.ok(!relay.allowedPathPrefixes.includes('/v1/chat/completions'), 'responses profile must not allow chat completions')
  assert.equal(relay.extraHeaders['user-agent'], 'codex_cli_rs/0.101.0')
  assert.equal(relay.extraHeaders.originator, 'codex_cli_rs')
  assert.equal(relay.extraHeaders.version, '0.101.0')
  ...
  const badHeader = runInstaller('model-key', 'profile', [
    '--model-key', `relay=${codexKey}`,
    '--model-base-url', 'relay=https://relay.example.com',
    '--model-header', 'relay=authorization=Bearer leaked',
  ], { MOCK_DOCKER_CONTAINER_EXISTS: '1' })
