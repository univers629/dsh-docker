...
const upstreams = [
  ...
  { name: 'relay', shape: 'responses', models: ['claude-opus-5-thinking'] },
...
try {
  ...
  assert.deepEqual(fresh.written.map((entry) => entry.name), ['deepseek', 'relay'])
  ...
  assert.match(fresh.settingsText, /apiKeyEnv: RELAY_API_KEY/)
  ...
  assert.deepEqual(fresh.credentialRefs.slice().sort(), ['DEEPSEEK_API_KEY', 'RELAY_API_KEY'])
  ...
  assert.deepEqual(again.credentialRefs.slice().sort(), ['DEEPSEEK_API_KEY', 'RELAY_API_KEY'])
  ...
  assert.deepEqual(merged.credentialRefs.slice().sort(), ['DEEPSEEK_API_KEY', 'RELAY_API_KEY'])
  ...
  const noEfforts = transform({
    brokerBase,
    placeholder,
    upstreams: [{ name: 'relay', shape: 'responses', models: ['claude-opus-5-thinking'] }],
    settingsText: '',
    credentialsText: '',
  })
