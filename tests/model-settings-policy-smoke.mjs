...
assert.equal(planProvider({ name: 'relay', shape: 'responses', models: ['x'], brokerBase: 'http://b:8080', catalog }).reasoningEfforts, null)
// 目录路由和第一方路由同样支持声明档位。
assert.deepEqual(
...
const seeded = planSeed({
  upstreams: [{ name: 'relay', shape: 'responses', models: ['claude-opus-5-thinking'], reasoningEfforts: ['low'] }],
  brokerBase: 'http://b:8080',
  placeholder: 'p',
  catalog,
})
