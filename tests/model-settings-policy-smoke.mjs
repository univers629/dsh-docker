...
assert.equal(deepseek.source, 'catalog')
assert.deepEqual(deepseek.always, { baseURL: 'http://b:8080/u/deepseek', apiKeyEnv: 'DEEPSEEK_API_KEY' })
...
const narrowed = planProvider({ name: 'deepseek', shape: 'any', models: ['deepseek-v4'], brokerBase: 'http://b:8080', catalog })
assert.deepEqual(narrowed.whenMissing, { models: [{ id: 'deepseek-v4' }] })
assert.deepEqual(narrowed.models, ['deepseek-v4'])
...
assert.deepEqual(fresh.defaultModel, { provider: 'deepseek', model: 'deepseek-v4-flash' })
