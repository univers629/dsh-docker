...
需要让 DSH Agent 以容器内 root 身份运行时，可在一行安装命令末尾显式加入参数：
...
Windows PowerShell 使用脚本块传参：

```powershell
& ([scriptblock]::Create((irm https://raw.githubusercontent.com/univers629/dsh-docker-dev/main/install.ps1))) -Root
```

`--root` 只让 DSH 主进程在容器内使用 UID 0，不会给容器增加 `privileged`、Docker Socket 或宿主机 root 权限；Nginx 仍以 `node` 用户运行。默认始终是普通 `node` 模式（这是容器内 UID 1000，不等同于宿主机当前登录用户）。root 模式写入 bind mount 的新文件时可能产生宿主机 root 所有的文件；再次使用 `--user` 启动时，入口会重新纠正 `/data` 和 `/workspace` 的属主。选择会写入工程目录 `.env` 的 `DSH_RUN_AS_ROOT`，后续更新和重启继续沿用。
...
- 脚本会显示包管理与配置校验的阶段性中文信息，并在当前 Agent 回复持久化完成后精确、优雅地重启 DSH，避免只看到红色的“工具结果未知”。唯一一次重启前提示会明确说明“包已写入但前端尚未确认生效”“将在本轮结束后重启”“不会在重启后补发通知”以及“等待 30～60 秒（复杂插件可能接近 1 分钟）后再刷新”；超过 90 秒仍未出现时再进入日志排查。
