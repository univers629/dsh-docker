...
case "$ACTION" in
  ...
  keys)
    if [ "$BROKER_ENABLED" != true ]; then
      ...
      echo "启用方法："
      echo "  1) 把上游密钥写进 data/broker/keys.json 并 chmod 600（这份文件是唯一的真实密钥来源）；"
      echo "  2) 在 .env 里设置 DSH_MODEL_BROKER=on；"
      echo "  3) 运行 $0 start，然后把 DSH 里的 base_url 改成 http://dsh-key-broker:8080/u/<name>/v1，密钥填占位值。"
      exit 0
    fi
