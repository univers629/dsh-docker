...
The panel is the browser alternative to typing keys in a terminal: add or remove upstreams, enter keys, pick the API shape, tick the model list with each model's capabilities and reasoning levels, set fixed request headers, set request limits, and fetch an upstream's model list once. Saving writes both `data/broker/keys.json` and DSH's `settings.yaml` and `.credentials.yaml`; both sides hot-reload, so no container restart is needed.
...
- Model list and per-model levels: the panel's model list is a table — only the rows you tick are saved into `keys.json` and written to DSH's `settings.yaml` (saving makes the table authoritative for the whole list, so unticking a model removes it from DSH's provider entry; ticking none of them for a catalog upstream falls back to the catalog's full list). Each row carries that model's own capabilities (`text` is the default and shown for information only; tick `image` to write `input: [text, image]`) and reasoning levels (`off`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`; `off` maps to `null`). Both are per-model facts: inside one gateway `gpt-5` accepts `reasoning_effort` while an image model does not. Declaring nothing means DSH shows no reasoning-effort dropdown for that model, and declaring levels for a model that does not accept `reasoning_effort` makes the upstream reject the request — check what the upstream actually supports. The chip row above the table applies levels to the ticked rows in bulk.
