...
```
--model-key relay=sk-...
--model-base-url relay=https://relay.example.com
--model-api relay=responses
--model-header relay=originator=codex_cli_rs
--model-header relay=version=0.101.0
--model-header relay=user-agent=codex_cli_rs/0.101.0
```
...
Omitting `requestsPerMinute` and `dailyRequestBudget` or setting them to `0` means unlimited, in which case the broker only prevents key exfiltration and provides no quota protection. Setting them explicitly is recommended (`requestsPerMinute: 60` is a reasonable start; size the daily budget from real usage and raise it after hitting 429). The admin panel's upstream form has two "request limits" fields (per-minute ceiling and daily budget): leave them blank to keep the current values, enter `0` to remove the limit. They can also be set with `--model-keys-file` or by editing `data/broker/keys.json` on the host directly (the broker hot-reloads it every 5 seconds by mtime). The install wizard does not ask for them, to keep the first run short. `maxRequestBytes` defaults to 8 MiB. `headerName` and `headerTemplate` default to `authorization` and `Bearer {key}`; only upstreams using a different auth header need them. `allowedPathPrefixes` defaults to the broker's built-in prefix set; setting it restricts forwarding to exactly those prefixes. On the client side, `base_url` is `http://dsh-key-broker:8080/u/<upstream>` with no version segment, and the api key field takes the placeholder `dsh-broker-placeholder`.
...
- Capabilities and reasoning levels are declared per model: pi-ai reports "no efforts offered" for every hand-declared model (all the installer and the panel write), so the dropdown never appears; conversely, declaring levels for a model that does not accept `reasoning_effort` makes the upstream reject the request. The panel's model list puts both on one table: only ticked rows reach the configuration, and each row carries `image` (writes `input: [text, image]`; `text` is the default and shown for information only) plus the level checkboxes (`off` maps to `null`, the other levels to their own names). Ticking no level declares nothing, and omitting the field in keys.json still means the same. Saving from the panel makes the table authoritative for the model list (the `settings.yaml` list is rewritten wholesale, and ticking nothing falls back to the catalog/default list), while the installer never touches the list — it only fills gaps, and inherits existing capabilities and levels by model id when the same upstream is reconfigured. The older behaviour on routes whose `models` list already exists is unchanged per field: missing fields are patched entry by entry and values the user wrote (including `reasoningEfforts: false`) are left alone.
...
When `.env` sets `DSH_KEY_ADMIN=on`, the installer also overlays `docker-compose.keys-admin.yml` and runs the separate `dsh-key-admin` container: a small panel that does nothing but model key configuration (add or remove upstreams, enter keys, pick the API shape, list model ids, set fixed request headers, set request limits, fetch an upstream's model list once). Saving writes both `data/broker/keys.json` and DSH's `settings.yaml` and `.credentials.yaml`. It also manages the container's egress policy by writing `data/egress/policy.json` (see "Egress modes").
...
The container is locked down like the broker: non-root (1000:1000), `cap_drop: ALL`, `no-new-privileges`, read-only root filesystem, `pids_limit`, and only `data/broker`, `data/dsh` and `data/egress` writable. Two details are worth stating separately:
...
`DSH_EGRESS_MODE` in `.env` selects how the container reaches the network: `open`, `blocklist`, or `allowlist`.
...
**blocklist** and **allowlist**: the installer overlays `docker-compose.isolated.yml` and all outbound traffic must pass the separate `dsh-egress` forward proxy (listening on 3128). Both modes share the same topology, address-form checks, port limits, and DNS validation; only the final domain decision differs. `blocklist` allows by default and denies domains matching the block list; `allowlist` denies by default and allows only domains matching the allow list. Topology:

```text
host 3080 ──► dsh-ingress ──► dsh-app:3080      Nginx L4 forwarding, the only container publishing a port
                    dsh ──► dsh-egress ──► internet   domain block/allow list, HTTP 80/443, CONNECT 443 only
...
- The `allowlist` built-in allow list has 15 domains: `deb.debian.org`, `security.debian.org`, `registry.npmjs.org`, `pypi.org`, `files.pythonhosted.org`, `github.com`, `api.github.com`, `codeload.github.com`, `objects.githubusercontent.com`, `raw.githubusercontent.com`, `github-releases.githubusercontent.com`, `pkg-containers.githubusercontent.com`, `ghcr.io`, `astral.sh`, `nodejs.org`.
- The `blocklist` built-in block list has 25 entries, all of them "one command publishes a container port to the internet" tunnel services: `*.trycloudflare.com`, five ngrok domains, `*.devtunnels.ms`, `*.loca.lt`, `*.lhr.life`, `*.serveo.net`, pinggy, zrok, bore, telebit, tunnelto, localtonet, jprq, `*.ts.net` (Tailscale Funnel), cpolar, natapp, natfrp, openfrp, `*.vicp.net`. What it stops is casual abuse — reverse-proxying `dsh-key-broker` to the internet so strangers spend the quota; frp on a self-owned domain or a personal VPS is not stopped. Every entry can be unchecked from the panel.
- In both modes, forms that bypass domain matching are refused by default: IP literals (including `127.0.0.1`, `169.254.169.254`, 10/8, 172.16/12, 192.168/16, `::1`, `fd00::/8`), `localhost` and the `.internal`/`.local`/`.localdomain`/`.localhost` suffixes, integer or hexadecimal address forms (`2130706433`, `0x7f000001`), URLs with credentials, non-http(s) schemes, and CONNECT to any port other than 443. Hop-by-hop headers are stripped on forward, and the proxy does not intercept TLS, so the in-container certificate chain is unchanged.
...
- Where the lists live: `data/egress/policy.json`, shaped as `{ version, mode, allowMode, allow: [{ host, enabled, note }], block: [...] }`. The file is mounted read-only into `dsh-egress` at `/etc/dsh-egress` and writable into the key admin panel container. The proxy hot-reloads it every 5 seconds by mtime (`DSH_EGRESS_POLICY_RELOAD_MS`, `0` reads once), keeps the previous ruleset and records a `policy-error` audit entry when parsing fails, and falls back to the environment variables if the file is removed. Adding or removing domains, toggling entries, and switching between `blocklist` and `allowlist` all happen in the panel's "container egress policy" section with no restart; only switching between `open` and isolated mode requires re-running the installer, because that changes the compose overlay.
- Environment fallback: without the policy file, rules come from `.env` (`DSH_EGRESS_MODE` and `DSH_EGRESS_ALLOWED_HOSTS`, comma separated, supporting leftmost single-label wildcards such as `*.example.com`, which does not match the bare domain, appended to the built-in allow list unless `DSH_EGRESS_ALLOWED_HOSTS_MODE=replace`), or from `--egress open|blocklist|allowlist` and `--egress-allow HOSTS` at install time. The file wins when it exists. A `mode` of `open` inside the file is read as `allowlist` (fail closed: if the proxy is running, the deployment is in isolated mode); a missing `block` field means "use the built-in tunnel list", an empty array means "block nothing". The dsh container does not mount this directory and cannot change it.
- Scope: under `allowlist`, anything outside the list returns 403 for apt, pip, npm, git, and for any web page or search API the agent tries to fetch, so required domains must be listed explicitly; under `blocklist`, only matching domains are refused and everything else works as usual. Neither mode affects model requests: that route does not go through `dsh-egress`, `dsh-key-broker` reaches upstreams directly.
...
- Operations: `./dsh.sh egress` prints the proxy `/status` (current mode, whether the rules came from the policy file or the environment, allow- and block-list sizes, allowed ports, whether resolution validation is on, active connections, allow and deny counters).
...
| `dsh-egress` | 1000:1000 | same | none | `bin` and `data/egress` read-only | Domain block/allow-list forward proxy |
...
- **The egress block list is heuristic**: it lowers the odds of a casual public tunnel, but frp on a self-owned domain or a personal VPS is not stopped; the only real egress boundary is `allowlist`.
...
- **Built-in control plugin**: the image ships `dsh-docker-control`, restored automatically on first start with an empty profile, which adds a "DSH environment" page to the settings navigation. Its authoritative source lives in the persistent data directory `/data/dsh/docker-control`; the baked `/opt/dsh-docker-control` is only a seed, copied in when that directory is unusable, when a reseed is requested, or when a new image ships a different seed. It is synced into the profile before every DSH start, so **editing the data-directory copy takes effect on restart with no image rebuild and no root**. Both seeding and syncing compare content hashes, warn loudly when overwriting a divergent copy, and record the provenance in `/run/dsh-state/docker-control-source.json`, which the `/info` `plugin` field and the `docker-control-source` check in `verify-dsh-hardening` both report. Setting `DSH_DOCKER_CONTROL_SOURCE` explicitly makes that path authoritative and leaves the data directory untouched. The plugin also offers a switchable container metrics card on the "DSH environment" page: it reads only this container's cgroup v2 CPU, memory working set and block I/O plus this network namespace's transferred bytes, sampled every two seconds into the bottom of the sidebar, with the switch stored per browser in `localStorage` and the polling stopped when it is off. The host-wide `/proc/stat` and `/proc/meminfo` are never read. Reads and writes share one loopback gate, but a same-origin GET/HEAD without `Origin` is allowed, because that is what browsers actually send — otherwise the page would be refused by its own `/info` and `/metrics`. Writes (restart, update, save configuration) still have to carry an `Origin` matching `Host`, so a cross-site request cannot drive them.
- **WebUI and proxy stability**: the configuration editor uses a dedicated portal and a fixed-height scroll area to avoid flicker and input height jumps, and the `dsh-docker-control` plugin adds a WebSocket keepalive to the event downlink, reducing UI hangs caused by idle reverse-proxy disconnects.
...
- **Plugins and toolchains**: the `/data` write access needed for plugin installation, session management, and MCP deployment is inside the sandbox. apt-installed software goes to standard Debian paths and persists in the container writable layer, while the Python and Node toolchains live in `/data/home/.local` and `/data/home/.npm-global`. The image deliberately sets no container-wide `HOME`: otherwise a host-side `docker exec`, which defaults to root, inherits the `dsh` home directory, and one root npm/npx run leaves root-owned files in the cache that make every later toolchain install fail with EACCES. The Supervisor passes `HOME` explicitly to the DSH child, the entrypoint rescans and repairs ownership on every start, and the privileged helper exposes one password-free `fix-perms` action (fixed paths, ownership restored only to the run account, `chown -Rh` so symlinks are not followed) so the Agent can recover without the container root password. The `container-environment` skill is rendered at container start from the actual system, architecture, and permission variables.
- **Plugin compatibility patches**: a few community plugins have known incompatibilities with the current DSH release. The container re-applies those compatibility patches whenever the plugin directory changes, so restart-free marketplace installs are covered too. The outcome is written to a report that `verify-dsh-hardening` reads as its `profile-patches` check: if an upstream change makes a patch anchor unrecognizable, it is reported as a warning instead of failing silently. One of those patches tells the plugin marketplace that the container Supervisor owns restarts (`allowRestart: false`, the marketplace's own documented switch for supervised deployments) and rewrites its "stop the current dsh process and run it again (`dsh web`)" hint to point at this project's restart entry — that advice would otherwise leave behind a DSH that is not a child of the Supervisor. The Supervisor carries a second, plugin-independent layer: before starting DSH it checks port 3081 and adopts whatever already serves it instead of starting a rival that cannot bind.
