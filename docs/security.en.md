...
- The reasoning-effort menu requires each model to declare its levels: pi-ai reports "no efforts offered" for every hand-declared model, which is all the installer and the panel write, so the dropdown never appears. The panel's reasoning-effort levels are checkboxes (`off`, `low`, `medium`, `high`, and `max` are checked by default for a new upstream; checking nothing declares nothing), and the checked levels write `reasoningEfforts` (`off` maps to `null`, the other levels to their own names) onto every model of that upstream, including routes whose `models` list was written earlier — those are patched entry by entry, and any model that already carries `reasoningEfforts` (including a user's own `false`) is left alone. An entry without that field in keys.json still declares nothing — the checkboxes are only the form's initial state: declaring levels for a model that does not accept `reasoning_effort` makes the upstream reject the request, so check what the upstream supports.
