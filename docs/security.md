...
- 调用能力与推理强度档位是逐模型声明的：pi-ai 对手写声明的模型（安装器和面板写出来的都是）一律报告"不提供任何档位"，模型页因此没有那个下拉；反过来，给不吃 `reasoning_effort` 的模型声明档位会被上游拒绝。管理面板的「模型清单」把这两件事放在同一张表的每一行上：勾上的模型才进配置，每行另有 `图像`（勾上写 `input: [text, image]`；`文本` 是默认能力、只作展示）与档位勾选框（`off` 的 wire 值是 `null`，其余用档位名）。一行都不勾 = 不声明，DSH 那边该模型就没有对应菜单；`keys.json` 里不写这个字段也依旧表示不声明。面板保存时模型清单以表里的为准（`settings.yaml` 整份覆盖，一个都不勾就退回目录/default 清单），安装器那条路不碰模型清单、只在缺失时补，并且重新配置同名上游时会按 id 把已有的能力与档位继承过来。上一次已经写过 `models` 的路由上逐条补字段的老口径仍然保留：只补缺，用户自己写过的值（包括 `reasoningEfforts: false`）不动。
...
- **插件兼容修补**：少数社区插件与当前 DSH 版本存在已知不兼容点，容器会在插件目录变化后自动重打这些兼容补丁，因此插件市场的免重启安装同样覆盖。结果写进一份报告，`verify-dsh-hardening` 的 `profile-patches` 一项会读它：上游改动导致补丁认不出代码形状时报 warn，而不是静默失效。其中一条通告插件市场“本 host 由容器 Supervisor 托管、一键重启权不在它手上”（`allowRestart: false`，市场文档里给被托管部署的正式开关），并把市场那句“关闭当前 dsh 进程后重新运行 `dsh web`”的重启提示改成指向本工程的重启入口——那句话在本容器里会造出一个不是 Supervisor 孩子的 DSH。Supervisor 另有一层不依赖插件配合的兜底：拉起 DSH 之前先看 3081 端口，谁在服务就接管谁，而不是再起一个去抢端口。
